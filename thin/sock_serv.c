	...
	for(;;) {
		...
		for( i = 0; i < maxfds; ++i) {
			...
			case 0: /* queue request */
				...
				data = &req->data;
                fprintf(stderr, "Request dequeued in dispatch queue\n");
	            print_payload(data);
				/* For the time being we use PAYLOAD_UNDEF as a way
				   to notify threads to exit
				*/
			...
			case 1: /* TCP socket */
                ...
                fprintf(stderr, "Payload received on netsocket accepted\n");
...
dispatch_hook(struct payload *data)
{
	...
	if ( data->reply != PAYLOAD_ACCEPTED ) {
        ...
        fprintf(stderr, "Master accepted payload\n");
...
parse_cmdline(int argc, char ** argv)
{
	...
	while ((arg = getopt(argc, argv, "dfs:")) != EOF ) {
        ...
        case 's':  /* start daemon in slave mode */
            fprintf(stderr,"Master Ip address passed as: %s\n", optarg);
...
add_vg(char *vg)
{
	...
	fprintf(stderr,"CLI: add_vg for %s\n",vg);
	...
	fprintf(stderr, "Successfully registered VG %s\n", p_vg->name);
	return 0;
out2:
	free(p_vg->r_queue);
out:
